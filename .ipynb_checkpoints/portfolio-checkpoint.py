...
@click.command()
@click.option('-s', '--cryptos', multiple=True, type=str, 
              default=['BTC', 'ETH', 'BNB', 'SOL', 'XRP', 'TRX', 'ADA', 'SHIB', 'LINK', 'DOT'], show_default=True,
              help='Crypto name to be included.'
              'When a file is provided, crypto name must be included in the file ')
@click.option('-b', '--budget', default=1000, show_default=True,
              help='Portfolio budget')
@click.option('-n', '--bin-size', type=int,
              help='Maximum number of intervals for each crypto. This a DQM-only option.')
...
@click.option('-f', '--file-path', default='data/crypto_basic_data.csv', 
              show_default=True, type=str,
              help='Full path of csv file containing input crypto data')
@click.option('-z', '--baseline', default='^GSPC', show_default=True,
              help='Baseline crypto for comparison in multi-period run')
...
@click.option('-d', '--dates', nargs=2, type=str,
              help='Start and end date to query crypto data from Yahoo! Finance')
...
@click.option('-k', '--num', default=0, type=click.IntRange(0, ),
              help='Number of cryptos to be randomnly generated.'
...
def main(cryptos, budget, bin_size, gamma, params, file_path, max_risk, num, 
    ...
    if rebalance:
        ...
        my_portfolio = MultiPeriod(cryptos=cryptos, budget=budget, 
                                    sampler_args=params, 
        ...
        my_portfolio = SinglePeriod(cryptos=cryptos, budget=budget,
