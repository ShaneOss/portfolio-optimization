...
class MultiPeriod(SinglePeriod):
    ...
    def __init__(self, cryptos=('BTC', 'ETH', 'BNB', 'SOL', 'XRP', 'TRX', 'ADA', 'SHIB', 'LINK', 'DOT'), budget=1000, 
        ...
        """
        super().__init__(cryptos=cryptos, budget=budget, t_cost=t_cost,
    ...
    def run(self, max_risk=0, min_return=0, num=0, init_holdings=None): 
        ...
        if not self.dates:
            ...
            self.dates = ['2017-01-01', formatted_date]
        self.load_data()
        ...
        headers = ['Date', 'Value'] + self.cryptos + ['Variance', 'Returns']
        self.opt_results_df = pd.DataFrame(columns=headers)
        row = []

        self.price_df = pd.DataFrame(columns=self.cryptos)
        ...
        for i in range(3, num_months):
            ...
            if first_purchase:
                ...
                budget = sum([df.iloc[-1][s]*result['cryptos'][s] for s in self.cryptos]) 
            ...
            if self.model_type == 'DQM':
                ...
                init_holdings = result['cryptos']

            # Print results to command-line
            value = sum([self.price[s]*result['cryptos'][s] for s in self.cryptos])
            returns = result['return']
            variance = result['risk']     

            row = [months[-1].strftime('%Y-%m-%d'), value] + \
                [result['cryptos'][s] for s in self.cryptos] + \
                [variance, returns] 
